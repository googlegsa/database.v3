...
public class Util {
  ...
  public static DBDocument generateMetadataURLFeed(String dbName,
    ...
    if (isWithBaseURL) {
      ...
      /*
       * build final document URL if docId is not null. Send null DBDocument if
       * document id is null.
       */
      if (docId != null) {
  ...
  /**
   * This method converts Large Object(BLOB or CLOB) into equivalent DBDocument.
   *
   * @param dbName
   * @param primaryKeys
   * @param row
   ...
   * @throws DBException
   */
  private static DBDocument setBlobContent(byte[] blobContent, DBDocument doc,
      String dbName, Map<String, Object> row, DBContext dbContext,
      String[] primaryKeys, TraversalContext context, String docId)
      throws DBException {
    /*
     * First try to get the MIME type of this file from the result set. If it
     * does not maintain a column for MIME type try to get the MIME type of this
     * document(file stored as BLOB in database) using MIME type finder utility.
     */
    String mimeType = "";
