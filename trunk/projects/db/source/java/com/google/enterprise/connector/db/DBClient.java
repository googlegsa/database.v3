 ...
 *
 * <p>It uses IBatis to talk to the database and generates SqlMapConfig and
 * SqlMap required by IBatis.
 *
 */
public class DBClient {
  ...
  public DBClient(DBContext dbContext, String sqlQuery,
      String googleConnectorWorkDir, String[] primaryKeys) throws DBException {
   ...
   * @return rows - subset of the result of executing the SQL query.
   *         E.g., result table with columns id and lastName and two rows will
   *         be returned as
   *         <pre>
   *         [{id=1, lastName=last_01}, {id=2, lastName=last_02}]
   *         </pre>
  ...
  public List<Map<String, Object>> executePartialQuery(int skipRows,
    ...
    LOG.info("Executing partial query with skipRows = " + skipRows + "and " +
        "maxRows = " + maxRows);
    try {
      rows = sqlMapClient.queryForList("IbatisDBClient.getAll", skipRows,
          maxRows);
    } catch (SQLException e) {
      throw new DBException("Could not execute query on the database\n", e);
    }
  ...
  /**
   * Generates the SqlMapConfig for mysql database.
   * It contains a reference to the SqlMap which should be a url or a file.
   * It assumes that the SqlMap is in IbatisSqlMap.xml in the
  ...
  private void generateSqlMapConfig() {
     ...
     * Also look into making DTD retrieving local with
    ...
    sqlMapConfig = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"
      ...
      + "\" />\n"
      + " <property name=\"JDBC.Password\" value=\"" + dbContext.getPassword()
      + "\" />\n"
  ...
  /**
   * Generates the SqlMap which contains the SQL query.
   * It writes the SqlMap in IbatisSqlMap.xml under googleConnectorWorkDir.
   *
   * @throws DBException
   */
