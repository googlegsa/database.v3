...
public class Util {
   ...
   * concatenated with a comma. Content is the xml representation of a row.
   * It also adds the checksum of the contents.
   *
  ...
  public static DBDocument rowToDoc(String dbName, String[] primaryKeys,
      Map<String, Object> row, String hostname, String xslt) throws DBException {
    ...
    doc.setProperty(SpiConstants.PROPNAME_DISPLAYURL, getDisplayUrl(hostname,
        dbName, docId));
    return doc;
  }
  ...
  /**
   * Generates the docId for a DB row.
   * If the primary keys are id and lastName and their corresponding values
   * are 1 and last_01, then the docId would be the SHA1 checksum of
   * (1,7)1last_01. The key values are concatenated and is prepended with
   * their lengths in parentheses.
   *
